...
for (const [tagName, encoded] of [["release/v1", "release%2Fv1"], ["v1 beta", "v1%20beta"]]) test("planner encoded release tag compatibility " + tagName, () => { const snapshot = makeGitHubSnapshot(); const release = snapshot.records.find(record => record.sourceType === "release"); release.payload.tagName = tagName; release.reference = "https://github.com/cyrilla-mist/nexus-ai/releases/tag/" + encoded; const plan = buildContextImportPlanV01(makePlannerInput("repository", { snapshot })); const candidate = plan.candidates.find(item => item.sourceRecordIds[0] === release.sourceRecordId); assert.equal(candidate.proposedPayload.claim, "GitHub release " + tagName + " is published."); assert.doesNotThrow(() => validateContextImportPlanV01(plan)); });
