...
test("provider summary counts each adapted ID once", () => { const p = structuredClone(packageValue()); p.identity.confirmed.push({ ...p.risks.open[0], source: { provider: "other", reference: "other" } }); const out = adaptGeneralizedContextPackageToV02(p); assert.equal(out.sourceSummary.totalIncludedNodes, 11); assert.equal(Object.values(out.sourceSummary.providers).reduce((sum, count) => sum + count, 0), out.sourceSummary.totalIncludedNodes); assert.equal(out.sourceSummary.providers.other, 1); });
